...
# --- launcher --------------------------------------------------------------------------
# The .app's main executable MUST be a native Mach-O binary: macOS on Apple Silicon
# treats a shell-script main executable as non-native and demands Rosetta. So the real
# logic lives in run.sh, and a tiny compiled launcher (packaging/launcher.c) execs it.
# Build-time variables (expanded now); runtime logic kept literal in the quoted heredoc.
cat > "$MACOS/run.sh" <<LAUNCHHEAD
#!/bin/bash
